...
func (c *FieldSecureAttrChangeConsumer) Process(ctx context.Context, evt domain.FieldSecureAttrChange) error {
	...
	chAny, loaded := c.workers.LoadOrStore(key, make(chan domain.FieldSecureAttrChange, 1))
	ch := chAny.(chan domain.FieldSecureAttrChange)
...
func (c *FieldSecureAttrChangeConsumer) runWorker(ctx context.Context, key string, ch chan domain.FieldSecureAttrChange) {
