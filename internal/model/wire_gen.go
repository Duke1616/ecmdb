...
func InitModule(db *mongox.DB, rmModule *relation.Module, attrModule *attribute.Module, resourceSvc *resource.Module) (*Module, error) {
	...
	defaultAttributeCreator := newAttrCreatorAdapter(service2)
	serviceService := service.NewModelService(modelRepository, v, defaultAttributeCreator)
	...
	handler := web.NewHandler(serviceService, mgService, relationModelService, encryptedSvc)
	module := &Module{
		Svc:   serviceService,
		MGSvc: mgService,
		Hdl:   handler,
