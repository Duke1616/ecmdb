...
func NewFieldDeleteConsumer(q mq.MQ, svc service.Service) (*FieldDeleteConsumer, error) {
	// NOTE: 在 mq.Consumer 初始化时订阅已删除字段事件 Topic
	consumer, err := q.Consumer(attribute.FieldDeleteEventName, "field_delete")
	if err != nil {
		return nil, fmt.Errorf("获取消息消费者失败: %w", err)
	}
...
func (c *FieldDeleteConsumer) Consume(ctx context.Context) error {
	...
	var evt attribute.FieldDeleteEvent
...
// Process 处理级联删除事件逻辑
func (c *FieldDeleteConsumer) Process(ctx context.Context, evt attribute.FieldDeleteEvent) error {
