...
func (e *attributeExecutor) ExecuteGroups(ctx context.Context, modelUid string, groups []domain.AttributeGroup) ([]domain.AttributeGroup, error) {
	if len(groups) == 0 {
	...
	groupsToCreate := slice.FilterMap(groups, func(idx int, src domain.AttributeGroup) (domain.AttributeGroup, bool) {
...
func (e *attributeExecutor) ExecuteFields(ctx context.Context, modelUid string, attrs []domain.Attribute) error {
	if len(attrs) == 0 {
		return nil
	}
	...
	// 使用 FilterMap 找出需要创建的分组（数据库中不存在的）
	attrsToCreate := slice.FilterMap(attrs, func(idx int, src domain.Attribute) (domain.Attribute, bool) {
		// 如果分组不存在于数据库中，返回 true
