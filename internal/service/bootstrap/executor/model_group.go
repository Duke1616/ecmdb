...
func (e *modelGroupExecutor) Get(ctx context.Context, groupName string) (domain.ModelGroup, error) {
	return e.modelGroupSvc.GetByName(ctx, groupName)
}

// Execute 执行模型分组创建
func (e *modelGroupExecutor) Execute(ctx context.Context, groups []domain.ModelGroup) ([]domain.ModelGroup, error) {
	if len(groups) == 0 {
	...
	// 获取需要新增的组名称
	names := slice.Map(groups, func(idx int, src domain.ModelGroup) string {
		return src.Name
	})
	...
	groupsToCreate := slice.FilterMap(groups, func(idx int, src domain.ModelGroup) (domain.ModelGroup, bool) {
