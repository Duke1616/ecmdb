...
import (
	...
	attributeSvc "github.com/Duke1616/ecmdb/internal/service/attribute"
	"github.com/Duke1616/ecmdb/internal/service/bootstrap/executor"
...
func (l *loader) loadModel(ctx context.Context, modelCfg structure.ModelConfig) error {
	...
	if _, err = l.modelExec.Execute(ctx, domain.Model{
		UID:     modelCfg.UID,
		Name:    modelCfg.Name,
		Icon:    modelCfg.Icon,
		GroupId: group.ID,
...
func (l *loader) loadAttributes(ctx context.Context, modelCfg structure.ModelConfig) error {
	...
	// 组合组数据
	groups := slice.Map(modelCfg.Attributes.Groups, func(idx int, src structure.AttributeGroupConfig) domain.AttributeGroup {
	...
	// 构建分组名称到ID的映射
	groupNameToID := slice.ToMap(attrGroups, func(element domain.AttributeGroup) string {
		return element.Name
	})
	...
	for _, groupCfg := range modelCfg.Attributes.Groups {
		...
		attr := slice.Map(groupCfg.Fields, func(idx int, src structure.FieldConfig) domain.Attribute {
...
func (l *loader) loadRelationTypes(ctx context.Context, relationTypes []structure.RelationTypeConfig) error {
	...
	rts := slice.Map(relationTypes, func(idx int, src structure.RelationTypeConfig) domain.RelationType {
...
func (l *loader) loadModelRelations(ctx context.Context, modelRelations []structure.ModelRelationConfig) error {
	...
	mrs := slice.Map(modelRelations, func(idx int, src structure.ModelRelationConfig) domain.ModelRelation {
...
func (l *loader) loadModelGroups(ctx context.Context, modelGroups []structure.ModelGroupConfig) (map[string]int64, error) {
	...
	groups := slice.Map(modelGroups, func(idx int, src structure.ModelGroupConfig) domain.ModelGroup {
	...
	// 模型组 key value 返回
	return slice.ToMapV(groupsResp, func(element domain.ModelGroup) (string, int64) {
