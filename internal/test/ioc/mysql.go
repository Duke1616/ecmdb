...
func InitMySQLDB() *gorm.DB {
	...
	if dsn == "" {
		dsn = "root:123456@tcp(localhost:3306)/cmdb?charset=utf8mb4&parseTime=True&loc=Local"
	}
...
func WaitForDBSetup(dsn string) {
	...
	for {
		...
		if !ok {
			panic(fmt.Sprintf("WaitForDBSetup 无法连接数据库: %v, 请检查本地 MySQL 是否启动或 DSN 是否正确", err))
		}
