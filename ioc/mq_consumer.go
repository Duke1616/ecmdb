...
func InitFieldSecureAttrChangeConsumer(q mq.MQ, svc resourceSvc.EncryptedSvc) (*resourceEvent.FieldSecureAttrChangeConsumer, error) {
	consumer := mqx.NewResilientConsumer(q, event.FieldSecureAttrChangeEventName, "field_secure_change")
	return resourceEvent.NewFieldSecureAttrChangeConsumer(consumer, svc, 100), nil
}
