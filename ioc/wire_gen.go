...
func InitApp() (*App, error) {
	...
	iStorage := storage.NewS3Storage(client)
	service9 := service5.NewService(iStorage)
	handler3 := web5.NewHandler(service9)
	iDataIOService := service6.NewService(serviceService, service7, service8)
	handler4 := web6.NewHandler(iDataIOService, iStorage)
	pluginDAO := dao.NewPluginDAO(db)
	pluginRepository := repository.NewPluginRepository(pluginDAO)
	iInputResolver := plugin.NewInputResolver(service7, relationResourceService)
	iSchemaImporter := plugin.NewSchemaImporter(service8, mgService, serviceService, relationTypeService, relationModelService)
	pluginService := plugin.NewService(pluginRepository, iInputResolver, iSchemaImporter, service8, mgService)
